...
public abstract class ElevationPDFWorker_generated extends java.lang.Object implements star.event.EventController, star.event.GatedListener, star.hydrology.events.ElevationPDFRaiser
{
	...
	public  ElevationPDFWorker_generated()
	{
		super();
	}
	...
	{
		getAdapter().addGatedAnd( new Class[]{ star.hydrology.events.map.FlowaccumulationMapLayerEvent.class,star.hydrology.events.map.FlowdirectionMapLayerEvent.class,star.hydrology.events.map.WatershedLayerEvent.class},new Class[]{ star.hydrology.events.ElevationPDFEvent.class},true);
	}
	 
	abstract void calculateElevation(star.hydrology.events.map.FlowaccumulationMapLayerRaiser flowAcc, star.hydrology.events.map.FlowdirectionMapLayerRaiser flowDir, star.hydrology.events.map.WatershedLayerRaiser watershed);
	...
	private void eventAndGateRaisedHandles(final star.event.Event in_event[], final boolean valid)
	{
		...
		{
			(new star.hydrology.events.ElevationPDFEvent(this,false)).raise();
			utils.Runner.runOnThread(new Runnable() { 
				public void run() { 
					long start = System.nanoTime();
					calculateElevation( (star.hydrology.events.map.FlowaccumulationMapLayerRaiser)event[0].getSource(),(star.hydrology.events.map.FlowdirectionMapLayerRaiser)event[1].getSource(),(star.hydrology.events.map.WatershedLayerRaiser)event[2].getSource() );
					long end = System.nanoTime();
				if( end - start > 500000000 ) { System.out.println( this.getClass().getName() + ".calculateElevation "  + ( end-start )/1000000 ); } }},this,1);
		}
	}
	...
	public void raise_ElevationPDFEvent()
	{
		(new star.hydrology.events.ElevationPDFEvent(this)).raise();
	}
	 
	public void removeNotify()
	{
		getAdapter().removeGatedAnd( new Class[]{ star.hydrology.events.map.FlowaccumulationMapLayerEvent.class,star.hydrology.events.map.FlowdirectionMapLayerEvent.class,star.hydrology.events.map.WatershedLayerEvent.class},new Class[]{ star.hydrology.events.ElevationPDFEvent.class});
	}
