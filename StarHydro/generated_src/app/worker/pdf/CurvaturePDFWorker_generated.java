...
public abstract class CurvaturePDFWorker_generated extends java.lang.Object implements star.event.EventController, star.event.GatedListener, star.hydrology.events.CurvaturePDFRaiser, star.hydrology.events.map.CurvaturePDFLMapLayerRaiser
{
	...
	public  CurvaturePDFWorker_generated()
	{
		super();
	}
	...
	{
		getAdapter().addGatedAnd( new Class[]{ star.hydrology.events.map.FlowaccumulationMapLayerEvent.class,star.hydrology.events.map.FlowdirectionMapLayerEvent.class,star.hydrology.events.map.WatershedLayerEvent.class},new Class[]{ star.hydrology.events.CurvaturePDFEvent.class,star.hydrology.events.map.CurvaturePDFLMapLayerEvent.class},true);
	}
	 
	abstract void calculateCurvature(star.hydrology.events.map.FlowaccumulationMapLayerRaiser flowAcc, star.hydrology.events.map.FlowdirectionMapLayerRaiser flowDir, star.hydrology.events.map.WatershedLayerRaiser watershed);
	...
	private void eventAndGateRaisedHandles(final star.event.Event in_event[], final boolean valid)
	{
		...
		{
			(new star.hydrology.events.CurvaturePDFEvent(this,false)).raise();
		(new star.hydrology.events.map.CurvaturePDFLMapLayerEvent(this,false)).raise();
			utils.Runner.runOnThread(new Runnable() { 
				public void run() { 
					long start = System.nanoTime();
					calculateCurvature( (star.hydrology.events.map.FlowaccumulationMapLayerRaiser)event[0].getSource(),(star.hydrology.events.map.FlowdirectionMapLayerRaiser)event[1].getSource(),(star.hydrology.events.map.WatershedLayerRaiser)event[2].getSource() );
					long end = System.nanoTime();
				if( end - start > 500000000 ) { System.out.println( this.getClass().getName() + ".calculateCurvature "  + ( end-start )/1000000 ); } }},this,1);
		}
	}
	...
	public void raise_CurvaturePDFEvent()
	{
		(new star.hydrology.events.CurvaturePDFEvent(this)).raise();
	}
	 
	public void raise_CurvaturePDFLMapLayerEvent()
	{
		(new star.hydrology.events.map.CurvaturePDFLMapLayerEvent(this)).raise();
	}
	 
	public void removeNotify()
	{
		getAdapter().removeGatedAnd( new Class[]{ star.hydrology.events.map.FlowaccumulationMapLayerEvent.class,star.hydrology.events.map.FlowdirectionMapLayerEvent.class,star.hydrology.events.map.WatershedLayerEvent.class},new Class[]{ star.hydrology.events.CurvaturePDFEvent.class,star.hydrology.events.map.CurvaturePDFLMapLayerEvent.class});
	}
