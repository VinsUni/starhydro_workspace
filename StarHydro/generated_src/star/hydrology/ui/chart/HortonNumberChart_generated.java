...
public abstract class HortonNumberChart_generated extends star.hydrology.ui.chart.AbstractChart implements star.event.EventController, star.event.Listener
{
	...
	private void eventRaisedHandles(final star.event.Event event)
	{
		if( event.getClass().getName().equals( "star.hydrology.events.StreamOrderStatisticsEvent" ) && event.isValid() ) 
		{
			...
			handleEvent( (star.hydrology.events.StreamOrderStatisticsRaiser)event.getSource());
		...
		}if( event.getClass().getName().equals( "star.hydrology.events.HortonNumberEvent" ) && event.isValid() ) 
		{
			 utils.Runner.runOnThread(new Runnable() { public void run() { 
			 long start = System.nanoTime();
			handleEvent( (star.hydrology.events.HortonNumberRaiser)event.getSource());
			 long end = System.nanoTime();
			 if( end - start > 500000000 ) { System.out.println( this.getClass().getName() + ".handleEvent "  + ( end-start )/1000000 ); } }},this,2);
		}
	}
	...
	abstract void handleEvent(star.hydrology.events.HortonNumberRaiser r);
	 
	abstract void handleEvent(star.hydrology.events.StreamOrderStatisticsRaiser r);
