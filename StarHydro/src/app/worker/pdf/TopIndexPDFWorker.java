...
import star.hydrology.events.TopIndexPDFEvent;
...
import star.hydrology.events.map.TopindexPDFMapLayerEvent;
...
public class TopIndexPDFWorker extends TopIndexPDFWorker_generated implements HeightsRange
{
	private float[] count;
	...
	private void calculate()
	{
		...
		if (getFlowAcc() != null && getWatershed() != null && getFlowDir() != null)
		{
					...
					if (!Float.isNaN(height) && !Float.isNaN(accomulation))
					{
						...
						if (!Float.isNaN(next_height))
						{
							...
							if (distance != 0)
							{
								...
								index = (float) Math.log(accomulation / (height_diff / distance));
								if (index > MIN_HEIGHT && index < MAX_HEIGHT)
								{
									if (index < min_height)
									{
										min_height = (int) index;
									}
									if (index > max_height)
									{
										max_height = (int) index;
									}
									count[(int) (index - MIN_HEIGHT)] += 1;
									layergrid.setElementAt(x, y, index);
								}
								else if (index < MIN_HEIGHT)
								{
									// count[(int) (MIN_HEIGHT - MIN_HEIGHT)] += 1;
								}
								else if (index > MAX_HEIGHT)
								{
									// count[(int) (MAX_HEIGHT - MIN_HEIGHT-1)] += 1;
								}
							}
						}
					}
			...
			layer.setLayerName("Top index PDF");
	...
	private void raise()
	{
		(new TopIndexPDFEvent(this, getLayer() != null)).raise();
		(new TopindexPDFMapLayerEvent(this, getLayer() != null)).raise();
	}
	...
	{
		return LayerConstants.TOPINDEXPDFLAYER;
	}

	@Handles(raises = { TopIndexPDFRaiser.class, TopindexPDFMapLayerRaiser.class }, concurrency = Handles.ASYNC)
	void calculateTopIndex(final FlowaccumulationMapLayerRaiser flowAcc, final FlowdirectionMapLayerRaiser flowDir, final WatershedLayerRaiser watershed)
	{
